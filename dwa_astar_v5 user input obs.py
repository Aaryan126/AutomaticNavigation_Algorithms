...
if show_animation:  # pragma: no cover
    ...
    plt.grid(True)
    plt.axis("equal")
    plt.title("Enter obstacles: left click to add, press Enter to proceed")
...
plt.grid(True)
plt.axis("equal")
plt.title("Path Planning After Adding Obstacles")

# Plot the obstacles again after entering DWA phase
for (x, y) in ob:
    circle = plt.Circle((x, y), config.robot_radius, color="k")
    plt.gca().add_patch(circle)
plt.plot(sx, sy, "og")
plt.plot(gx, gy, "*b")
plt.plot(rx, ry, "xb")  # Plot the A* path
plt.pause(0.001)  # Add a brief pause to see the plot
