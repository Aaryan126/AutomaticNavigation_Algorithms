...
def on_click(event):
    ...
    if event.button == 2:  # Left click
        if event.xdata is not None and event.ydata is not None:
            ...
            new_obstacle = np.array([[event.xdata, event.ydata]])
            ob = np.append(ob, new_obstacle, axis=0)
            print(f"Added obstacle at: {event.xdata}, {event.ydata}")  # Debug statement
            circle = plt.Circle((event.xdata, event.ydata), 0.5, color="k")  # Adjust radius as needed
            plt.gca().add_patch(circle)
            plt.draw()
...
visited_local_goals = set()

for i_goal, dwagoal in enumerate(road_map):
    if i_goal == 0:  # Skip the start point
        continue

    # Convert local goal to a tuple for easy set operations
    goal_tuple = (round(dwagoal[0], 2), round(dwagoal[1], 2))  # Rounding for precision

    # Check if the local goal has already been visited
    if goal_tuple in visited_local_goals:
        print(f"Skipping local goal {dwagoal} as it has already been visited.")
    ...
    # Mark this local goal as visited
    visited_local_goals.add(goal_tuple)
